...
class Hand(object):
    ...
    def update(self, word):
        ...
        for key in word_freq:
            self.handDict[key] -= word_freq[key]
    ...
    def containsLetters(self, letters):
        ...
        return all(key in self.handDict.keys() and
                   letters_freq[key] <= self.handDict[key]
    ...
    def __eq__(self, other):
        ...
        # TODO
        return all(key in self.handDict.keys() and
                   other.handDict[key] == self.handDict[key]
...
class ComputerPlayer(Player):
    ...
    PickBestWord method.
    """
    def __init__(self, idNum, hand, wordlist):
        super(ComputerPlayer, self).__init__(idNum, hand)
        self.dicc = {}
        for word in wordlist.getList():
            key = ''.join(sorted(word))
            if key in self.dicc.keys():
                self.dicc[key].append(word)
            else:
                self.dicc[key] = [word]
        keys = self.dicc.keys()
        keyscore_tuples = [(key, getWordScore(key)) for key in keys]
        # sort the tuples in descending order based on score
        self.keyscore_tuples = sorted(keyscore_tuples, key=itemgetter(1), reverse=True)
    ...
    def pickBestWord(self):
        ...
        if len(self.keyscore_tuples) == 0:
            return '.'
        wordlen = min(hand_length, len(self.keyscore_tuples[0][0]))
        # removing elements having word length greater than wordlen
        while wordlen < len(self.keyscore_tuples[0][0]):
            del self.keyscore_tuples[0]
        # finding the  right word
        while len(self.keyscore_tuples) > 0:
            maxscore_element = self.keyscore_tuples[0]
            if not self.hand.containsLetters(maxscore_element[0]):
                self.keyscore_tuples.remove(maxscore_element)
                continue
            for word in self.dicc[maxscore_element[0]]:
    ...
    def playHand(self, callback, wordlist):
        ...
        function.
        """
        while callback(self.pickBestWord()): pass
...
class Game(object):
    ...
    def __init__(self, mode, wordlist):
        ...
        if mode == HUMAN_SOLO:
            ...
            self.players = [HumanPlayer(1, hand),
                            ComputerPlayer(2, hand2, wordlist)]
